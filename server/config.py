...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        ...
        extra='ignore',   # ignore keys from old .env files
    )
    ...
    license_secret:      str = Field('changeme', alias='LICENSE_SECRET')
    cors_origins:        str = Field('http://localhost:3000', alias='CORS_ORIGINS')
    server_port:         int = Field(8000, alias='SERVER_PORT')

    # Firebase (service account JSON as env var — see docs/FIREBASE_SETUP.md)
